...
def optical_rabi_resonant_fit(datafile, pulse_start, pulse_end, 
    ...
    if save:
        figure.savefig(os.path.join(os.path.split(datafile)[0],\
                os.path.split(datafile)[1] + '_optical_rabi_fit.png'))

    return omegaR



def optical_rabi_resonant_fit_simple(datafile, pulse_start, pulse_end, 
        use_channel = 1, save = True, max_rabi_amp = 0.9, binsize=256):
    """
    can process npz files and txt files.
    use channel specifies the HH channel, can be either 0 or 1.
    ...
    fit_result = fit.fit1d(time_fit, counts_fit, rabi.optical_rabi_damped, 
            omegaR_init_guess, xi_init_guess, time_fit[0], ampl_init_guess, 
            offset_init_guess, do_plot = True, newfig = False, ret = True,
